...
type
  TFrmMain = class(TForm)
    ...
    ImportfromimageMagickaVoxel1: TMenuItem;
    ...
    PasteMagickaVoxelImage1: TMenuItem;
    ImageFromMagickaVoxel1: TMenuItem;
    procedure ImageFromMagickaVoxel1Click(Sender: TObject);
    ...
    procedure ImportfromimageMagickaVoxel1Click(Sender: TObject);
    ...
    procedure PasteMagickaVoxelImage1Click(Sender: TObject);
  private
...
uses FormHeaderUnit, LoadForm, FormNewSectionSize, FormPalettePackAbout, HVA,
   ...
   Voxel_Engine, FormNewSectionImage, FormImportSlices, CommunityLinks, pngimage;
...
procedure TFrmMain.PasteMagickaVoxelImage1Click(Sender: TObject);
var
   Frm: TFrmImportSlices;
   image: TPngObject;
   x,y,z: integer;
   horizontal: boolean;
   i: Integer;
...
begin
   ...
   {$ifdef DEBUG_FILE}
   DebugFile.Add('FrmMain: PasteMagickaVoxelImage1Click');
   {$endif}

   Frm := TFrmImportSlices.Create(self);
   with Frm do
   begin
      ShowModal;
      if aborted then Exit;

      if FileExists(Frm.EdImage.Text) then
      begin
         image := TPNGObject.Create;
         image.LoadFromFile(Frm.EdImage.Text);
         ...
         begin
            MessageDlg('Error! Dimensions of the section are out of bonds. Try using a different image',mtError,[mbOK],0);
            exit;
         end;

         if MessageDlg('Paste Image from MagickaVoxel' + #13#13+ 'Are you sure you want to paste this image? This will override voxel data in active section!' + #13#13 +
             'Size: ' + IntToStr(X) + 'x' + IntToStr(Y) + 'x' + IntToStr(Z),
             mtConfirmation,[mbYes,mbNo],0) = mrNo then
         exit;
         ...
         ImageIntoVXL(Document.ActiveSection^, image, offset, horizontal);
         SetVoxelChanged(true);
         RefreshAll;
         Image.Free;
      end;
...
procedure TFrmMain.ImageFromMagickaVoxel1Click(Sender: TObject);
var
   Frm: TFrmNewSectionImage;
   image: TPngObject;
   x,y,z: integer;
   horizontal: boolean;
   i, SectionIndex: Integer;
begin
   {$ifdef DEBUG_FILE}
   DebugFile.Add('FrmMain: ImageFromMagickaVoxel1Click');
   {$endif}

   Frm := TFrmNewSectionImage.Create(self);
   Frm.Caption := 'Import Image from MagickaVoxel';
   with Frm do
   begin
      lblPosition.Visible := false;
      chkBefore.Visible := false;
      chkAfter.Visible := false;
      ShowModal;
      if aborted then Exit;
      ...
      begin
         ...
         CheckVXLChanged;
         VoxelType := vtAir;
         if (NewVoxel(Document,4,x,y,z))then
         begin
            VoxelOpen := false;
            VXLChanged := false;
            for i:=1 to 16 do
               Document.ActiveSection^.Header.Name[i] := Name[i];
...
procedure TFrmMain.ImportfromimageMagickaVoxel1Click(Sender: TObject);
var
   Frm: TFrmNewSectionImage;
   image: TPngObject;
   x,y,z: integer;
...
begin
   ...
   {$ifdef DEBUG_FILE}
   DebugFile.Add('FrmMain: ImportFromImage1Click');
   {$endif}
   ...
   begin
      ...
      begin
         ...
         if MessageDlg('Create Section from an Image (MagickaVoxel)' + #13#13+ 'Are you sure you want to create a section: ' + #13#13 +
             'Name: ' + AsciizToStr(Name,16) + Chr(10) +
             'Size: ' + IntToStr(X) + 'x' + IntToStr(Y) + 'x' + IntToStr(Z),
             mtConfirmation,[mbYes,mbNo],0) = mrNo then
         exit;
