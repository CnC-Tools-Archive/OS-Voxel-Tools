...
procedure ImageIntoVXL(var _Vxl : TVoxelSection; _Image : TPngObject; _Offset : integer; _Horizontal : boolean = false);
...
begin
   ...
   {$ifdef DEBUG_FILE}
   FrmMain.DebugFile.Add('VoxelEngine: ImageIntoVXL');
   {$endif}
      ...
      for y := 0 to _Vxl.Tailer.YSize-1 do
         for z := 0 to _Vxl.Tailer.ZSize-1 do
            ...
            begin
               ...
               if _Horizontal then
                  localX := localX + _Offset * (_Vxl.Tailer.YSize-1 - y)
               else
                  localY := localY + _Offset * (_Vxl.Tailer.YSize-1 - y);

               if (localX < _Image.Width) and (localX >= 0) and (localY < _Image.Height) and (localY >= 0) then
               begin
                  if (_Image.AlphaScanline[localY]^)[localX] = 0 then
                     v.Used := false
                  else
                     v.Used := true;

                  v.Colour := FrmMain.Document.Palette^.GetColourFromPalette(_Image.Pixels[localX,localY]);

                  if v.Used = true then
                     _Vxl.SetVoxel(x,y,z,v);
               end;
            end;
   end;
end;
